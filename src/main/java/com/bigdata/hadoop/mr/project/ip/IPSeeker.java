package com.bigdata.hadoop.mr.project.ip;
...
public class IPSeeker {
	...
	private final byte REDIRECT_MODE_1 = 0x01;
	private final byte REDIRECT_MODE_2 = 0x02;
	...
	public String getCountry(byte[] ip) {
		...
		if (ipCache.containsKey(ipStr)) {
			...
			log.debug("get ip country from ipCache ");
			return ipLoc.getCountry();
		} else {
			IPLocation ipLoc = getIPLocation(ip);
			ipCache.put(ipStr, ipLoc.getCopy());
			log.debug("get ip country and set ip country to ipCache  ");
			return ipLoc.getCountry();
		}
	}
	...
	public String getArea(byte[] ip) {
		...
		if (ipCache.containsKey(ipStr)) {
			IPLocation ipLoc = ipCache.get(ipStr);
			log.debug("get ip area from ipCache ");
			return ipLoc.getArea();
		} else {
			IPLocation ipLoc = getIPLocation(ip);
			ipCache.put(ipStr, ipLoc.getCopy());
			log.debug("get ip area and set ip area to ipCache  ");
			return ipLoc.getArea();
		}
	}
